...
- In the SAT complexity transition we can see that a transition from a single peak to clusters of peaks. Monte Carlo algorithms wont do well in such "glassy" configuration.
* Flashcards
